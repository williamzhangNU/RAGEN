...
def get_rl_train_command(config: Dict[str, Any]) -> str:
    ...
    cmd = [
        ...
        f"+trainer.val_only={str(config['training']['val_only']).lower()}",
        f"+trainer.val_before_train={str(config['training']['val_before_train']).lower()}",
        f"trainer.default_hdfs_dir={config['training']['default_hdfs_dir'] or 'null'}",
        f"trainer.n_gpus_per_node={config['system']['n_gpus']}",
        f"trainer.nnodes={config['training']['nnodes']}",
        f"trainer.save_freq={config['training']['save_freq']}",
        f"trainer.test_freq={config['training']['test_freq']}",
        f"trainer.project_name={config['training']['project_name']}",
        f"trainer.experiment_name={config['exp_name']}",
